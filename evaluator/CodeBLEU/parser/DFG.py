...
def DFG_rust(root_node,index_to_code,states):
    ...
    def_statement=['let_declaration']
    increment_statement=['compound_assignment_expr']
    if_statement=['if_expression','else_clause']
    for_statement=['for_expression']  # rust has only for-each loop, like python
    while_statement=['while_expression']
    do_first_statement=[]
    states=states.copy()
    if (len(root_node.children)==0 or root_node.type in ['string_literal','string','char_literal']) and root_node.type!='comment':
        idx,code=index_to_code[(root_node.start_point,root_node.end_point)]
        if root_node.type==code:
            return [],states
        elif code in states:
        ...
        name=root_node.child_by_field_name('pattern')
        value=root_node.child_by_field_name('value')
        DFG=[]
        if value is None:
            indexs=tree_to_variable_index(name,index_to_code)
        ...
        for i in range(2):
            ...
            for left_node,right_node in zip(left_nodes,right_nodes):
                ...
                DFG+=temp
            if  root_node.children[-1].type=="block":
                temp,states=DFG_rust(root_node.children[-1],index_to_code,states)
                DFG+=temp
        dic={}
        for x in DFG:
            if (x[0],x[1],x[2]) not in dic:
